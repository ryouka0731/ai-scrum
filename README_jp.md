...
顧客エージェント（イツキ）が最新の `orderXXX.md` を読み込み、内容についてユーザに質問します。  
...
- 顧客エージェント（イツキ）がフィードバック（機能性・ユーザビリティ・ビジネス価値）を提供
- プロダクトオーナー（シュリ）が各PBIの受入判定を実施
...
### プロダクトオーナーに質問する — `/ask-to-po-shuri`

```
/ask-to-po-shuri
```

プロダクトオーナー（シュリ）との対話型セッションを開始します。  
...
プロダクトオーナー（シュリ）がリクエストを受け取り、適切なサブエージェント（開発者、スクラムマスター、レビュー担当）にタスクを割り振ります。レビュー担当（ヒツギ）の指摘がなくなるまで反復し、ユーザのリクエストに回答します。
